...
export class TodoRepository extends Repository<Todo> {
  ...
  filter(values: Record<string, any>): Knex.QueryCallback {
    return (builder: Knex.QueryBuilder) => {
      ...
      if (hasOwnProperty(values, 'is_late')) {
        if (values.is_late) {
          builder.whereNull('done_at');
          builder.where('due_at', '<', new Date());
        }
      }
      ...
      if (hasOwnProperty(values, 'done_at_from')) {
        builder.where('done_at', '>', values.done_at_from);
      }

      if (hasOwnProperty(values, 'done_at_to')) {
        builder.where('done_at', '<', values.done_at_to);
      }
    };
  }
}
